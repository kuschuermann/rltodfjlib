...
/**
 * <p>
 * Supports parsing and maintains a mapping of the resulting named
 * {@linkplain Entry entries} comprising the {@link Container}.
 * </p>
 * 
    ...
    Iterable<Entry>
{
	...
	if( pf == null )
	  {
	    ...
	    pf.setNamespaceAware( false ); // we want to see the names with ':'
		                           // in them
	    pf.setValidating( false );
	    pf.setFeature( "http://xml.org/sax/features/validation",
		           false );
	    pf.setFeature( "http://apache.org/xml/features/validation/schema",
		           false );
	    pf.setFeature( "http://apache.org/xml/features/nonvalidating/load-dtd-grammar",
		           false );
	    pf.setFeature( "http://apache.org/xml/features/nonvalidating/load-external-dtd",
		           false );
	  }
      }
	                  ...
	                  final String qName )
  {
	 ...
	 * meta.xml manifest:size 2003 manifest:encryption-data
	 * manifest:checksum-type
	 * urn:oasis:names:tc:opendocument:xmlns:manifest:1.0#sha256-1k
	 * manifest:checksum (base64-encoded binary) manifest:algorithm
	 ...
	 * manifest:key-size 32 manifest:iteration-count 1024 manifest:salt
	 * (base64-encoded binary) manifest:start-key-generation
	 * manifest:start-key-generation-name
	 * http://www.w3.org/2000/09/xmldsig#sha256 manifest:key-size 32
	 */
	...
	if( cAttr == null )
	...
	else
	  {
		...
		crypto = new Crypto( cAttr,
	      ...
	      {
		final IllegalStateException boom = new IllegalStateException( "Unsupported crypto" );
		boom.initCause( x );
		throw boom;
	      }
